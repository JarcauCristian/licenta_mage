...
def load_data(*args, **kwargs):
    ...
    if None in initial_name:
        raise ValueError("Kwarg initial_name required!")

    config = config_loader()

    response = requests.get(f'{config["default"]["MINIO_API"]}/balancer/get_object?dataset_path={initial_name}')

    if response.status_code != 200:
        raise ValueError('Failed to information about the saved dataset.')
